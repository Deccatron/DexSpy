...
{
    ["Main"] = {
        ["HeaderColor"] = Color3.fromRGB(40, 40, 40),  -- Dark Gray
        ["HeaderShadingColor"] = Color3.fromRGB(60, 60, 60),  -- Darker Gray
        ["HeaderTextColor"] = Color3.fromRGB(255, 165, 0),  -- Soft Orange
        ["MainBackgroundColor"] = Color3.fromRGB(30, 30, 30),  -- Very Dark Gray
        ["InfoScrollingFrameBgColor"] = Color3.fromRGB(30, 30, 30),  -- Very Dark Gray
        ["ScrollBarImageColor"] = Color3.fromRGB(100, 110, 130)  -- Softer Contrast
    },
    ["RemoteButtons"] = {
        ["BorderColor"] = Color3.fromRGB(255, 140, 0),  -- Dark Orange
        ["BackgroundColor"] = Color3.fromRGB(50, 50, 50),  -- Darker Slate Grey
        ["TextColor"] = Color3.fromRGB(240, 240, 240),  -- Off-White
        ["NumberTextColor"] = Color3.fromRGB(255, 165, 0)  -- Soft Orange
    },
    ["MainButtons"] = { 
        ["BorderColor"] = Color3.fromRGB(255, 140, 0),  -- Dark Orange
        ["BackgroundColor"] = Color3.fromRGB(50, 50, 50),  -- Darker Slate Grey
    ...
    ['Code'] = {
        ['BackgroundColor'] = Color3.fromRGB(30, 35, 43),  -- Darker Background
        ['TextColor'] = Color3.fromRGB(240, 240, 240),  -- Off-White
        ['CreditsColor'] = Color3.fromRGB(200, 200, 200)  -- Light Grey
    },
}
...
if not isfile("DexSpySettings.json") then
    writefile("DexSpySettings.json", HttpService:JSONEncode(settings))
else
    if HttpService:JSONDecode(readfile("DexSpySettings.json"))["Main"] then
        writefile("DexSpySettings.json", HttpService:JSONEncode(settings))
    else
        settings = HttpService:JSONDecode(readfile("DexSpySettings.json"))
    end
end

-- Compatibility for protosmasher:
...
-- delete the previous instances of DexSpy
if game.CoreGui:FindFirstChild("DexSpyGUI") then
    game.CoreGui.DexSpyGUI:Destroy()
end
...
local DexSpyGUI = Instance.new("ScreenGui")
...
DexSpyGUI.Name = "DexSpyGUI"

Parent(DexSpyGUI)

mainFrame.Name = "mainFrame"
mainFrame.Parent = DexSpyGUI
mainFrame.BackgroundColor3 = Color3.fromRGB(60, 60, 60)  -- Updated to slate grey
mainFrame.BorderColor3 = Color3.fromRGB(60, 60, 60)  -- Updated to slate grey
...
BrowserHeader.Parent = DexSpyGUI
BrowserHeader.BackgroundColor3 = colorSettings["Main"]["HeaderShadingColor"]  -- Header shading color
...
BrowserHeaderFrame.BackgroundColor3 = colorSettings["Main"]["HeaderColor"]  -- Header color
...
RemoteBrowserFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)  -- Slate Grey
RemoteBrowserFrame.BorderColor3 = Color3.fromRGB(40, 40, 40)  -- Slate Grey
...
-- (After the RemoteBrowserFrame is created)

-- Step 1: Create a search TextBox
local SearchBox = Instance.new("TextBox")
SearchBox.Name = "SearchBox"
SearchBox.Parent = BrowserHeader
...
-- Step 2: Create the filtering function
local function filterRemotes(searchTerm)
    searchTerm = searchTerm:lower() -- Convert to lowercase for case-insensitive comparison
    for i, button in ipairs(RemoteScrollFrame:GetChildren()) do
        if button:IsA("TextButton") and button.Visible then
            local remoteName = button.RemoteName.Text:lower() -- Use lower case for comparison
            button.Visible = remoteName:find(searchTerm) ~= nil -- Show or hide based on comparison
        end
    end
end

-- Step 3: Connect the search TextBox to the filter function
...
mouse.KeyDown:Connect(function(key)
    if key:lower() == settings["Keybind"]:lower() then
        DexSpyGUI.Enabled = not DexSpyGUI.Enabled
    end
end)

Header.Name = "Header"
Header.Parent = mainFrame
Header.BackgroundColor3 = colorSettings["Main"]["HeaderColor"]  -- Header Color
...
HeaderShading.BackgroundColor3 = colorSettings["Main"]["HeaderShadingColor"]  -- Header Shading Color
...
HeaderTextLabel.Text = "DexSpy"  -- Changed to "Dex Spy"
...
RemoteScrollFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 40)  -- Slate Grey
RemoteScrollFrame.BorderColor3 = Color3.fromRGB(40, 40, 40)  -- Slate Grey
...
Code.Text = "Thanks for using DexSpy!"  -- Changed to "Dex Spy"
...
CodeComment.Text = "-- Remote Spy Developed by Deccatron"  -- Changed to "Dex Spy"
...
CopyCode.MouseButton1Click:Connect(function()
    ...
    -- copy the code's text to clipboard if the user is looking at a remote
...
CopyDecompiled.MouseButton1Click:Connect(function()
    ...
    if not decompiling and remote and lookingAt then
        ...
        -- Decompile the remote script of the remote
...
-- detect when a child is added to the remotescrollframe and add a mousebutton1click signal
...
function addToList(event, remote, ...)
    ...
    if not i then
        ...
        -- clone a little baby of the remote button
        ...
        -- update the code box if the player is looking at it
